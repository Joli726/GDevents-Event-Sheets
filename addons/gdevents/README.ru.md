...
**Версия 0.9.2** ([что нового](CHANGELOG.md), лицензия MIT): редактор листов, 34 готовых поведения, 244 инструкции,
...
- **Значения правятся прямо в листе**, как в GDevelop: щелчок по
  подсвеченному значению — объект и знак выбираются из списка, число и текст
  вписываются в поле у курсора (Enter — готово, Escape — отмена).
- **«Добавить событие» под последним событием** — новое пустое событие в
  конец листа, как в GDevelop (или Ctrl+N).
...
**Снятое поведение уносит свой каркас.** Узлы, которые поведение создало
само, удаляются вместе с ним. Каркас остаётся целиком, если в него положили
что-то своё или каким-то его узлом пользуется другое поведение объекта —
например, спрайтом платформера пользуется «Сочность»: половина каркаса хуже
любого из двух, тело без формы падает сквозь пол. У поведений, поставленных
до версии 0.9.1, записи о каркасе нет — их узлы остаются, удалите их руками.
...
Четырнадцатый, `tools/bench.tscn`, — нагрузка: 500 врагов и 300 пуль,
всё движется, пули бьют врагов, «только что столкнулся» и «касается
сверху», с одними формами и с Area2D; печатает время кадра листа и падает,
если оно больше 33 мс (прежняя проверка «каждый с каждым» здесь занимала
1700 мс, теперь — около 5–15 мс).
